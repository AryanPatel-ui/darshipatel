...
export default function Hero() {
  ...
  return <section className="hero-stage" id="hero"><div className="hero-arch-decor" /><div className="spotlight-container">{['far-left','inner-left','center','inner-right','far-right'].map(c => <div className={`spotlight-beam ${c}`} key={c} />)}</div><div className="stage-floor-glow" /><div className="hero-content"><div className="hero-masks-badge"><Icon name="masks" /></div><span className="hero-pre">Act I — The Opening Act</span><h1 className="hero-title">Darshi <span className="gold-text">Patel</span></h1><p className="hero-tagline">Architecting immersive brand narratives, live event dynamics, and experiential content that commands the audience's attention.</p></div><div className="hero-images-showcase">{images.map(([size, src, alt]) => <div className={`hero-img-frame ${size}`} key={alt}><a href="https://www.instagram.com/_darshi_/" target="_blank" rel="noopener noreferrer" className="hero-img-link" title="Visit Darshi on Instagram"><img src={src} alt={alt} /></a></div>)}</div><NextChapter to="/about" act="Act II" title="Curiosity in Motion" icon="ticket" /></section>;
}
